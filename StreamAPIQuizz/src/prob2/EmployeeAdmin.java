...
public class EmployeeAdmin {
	...
	public static List<String> prepareSsnReport(HashMap<String, Employee> table, List<String> socSecNums) {
		//implement
		//return table.keySet().stream().filter(k->(!socSecNums.contains(k))).sorted((a,b)->Integer.parseInt(a)-Integer.parseInt(b)).collect(Collectors.toList());
		Function<String,Integer> fun=(a)->Integer.parseInt(a);
		return table.keySet().stream().filter(k->(!socSecNums.contains(k))).sorted(Comparator.comparing(fun)).collect(Collectors.toList());		
	}			
	...
	public static List<Employee> prepareEmpReport(HashMap<String, Employee> table, List<String> socSecNums) {
		//implement
		return table.values().stream().filter(e->socSecNums.contains(e.getSsn())).filter(e->e.getSalary()>80000).collect(Collectors.toList());
	}
}